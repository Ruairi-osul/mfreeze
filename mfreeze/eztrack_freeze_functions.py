...
def LoadAndCrop(
    ...
    image.opts(
        width=int(frame.shape[1] * stretch["width"]),
...
def PlayVideo(
    ...
    if display_dict["save_video"] == True:
        ...
        if not save_dir:
            save_dir = os.path.normpath(video_dict["dpath"])
        if not fname:
            fname = "video_output.avi"
        fname = os.path.join(save_dir, fname)
        writer = cv2.VideoWriter(fname, fourcc, 20.0, (width, height), isColor=False,)
...
def Batch(
    video_dict, bin_dict, mt_cutoff, FreezeThresh, MinDuration, crop=None, SIGMA=1
):
    """ 
    -------------------------------------------------------------------------------------
...
def Calibrate(video_dict, cal_pix, SIGMA, start_frame):
    """ 
    -------------------------------------------------------------------------------------
    ...
    cal_frames = int(video_dict["cal_sec"] * video_dict["fps"])
